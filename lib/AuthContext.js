...
export function AuthProvider({ children }) {
  ...
  // Bumped every time the session ends (logout, or the unauthorized event).
  // A scheduled refresh captures the epoch it was scheduled under; if that
  // epoch has moved by the time the refresh's await resolves, the session
  // ended while it was in flight and its result must be ignored — otherwise
  // a refresh that was already sent before logout can resolve afterward and
  // silently revive `user` even though the app told the server to log out.
    ...
    (expiresAt) => {
      ...
      timerRef.current = setTimeout(async () => {
        try {
          ...
          if (sessionEpochRef.current !== epochAtSchedule) return;
          setUser(data.user);
  ...
  useEffect(() => {
    ...
    if (session?.user && session.expiresAt > Date.now()) {
      ...
      apiMe().then((data) => {
        // The cached copy (sessionStorage) only exists for an instant first
        // render and is not trusted data — it's readable/writable from the
        // browser console. Always overwrite it with the server's answer, so
        // a stale or tampered cached role never outlives this one round-trip.
