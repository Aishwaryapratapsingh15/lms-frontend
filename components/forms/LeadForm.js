...
export default function LeadForm({ salesUsers = [], showAssignee, initialValues, onSubmit, onCancel }) {
  ...
  return <form onSubmit={handleSubmit} className="space-y-4">
    ...
    <div className="grid grid-cols-2 gap-3">
      <div><label className="mb-1 block text-sm font-medium text-slate-700">City</label><input className={FIELD_CLASS} value={form.city ?? ""} onChange={(e) => update("city", e.target.value)} /></div>
