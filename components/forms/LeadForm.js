...
const EMPTY = { fullName: "", email: "", phone: "", company: "", source: LEAD_SOURCES[0], status: "NEW", priority: "MEDIUM", notes: "", assignedToId: "" };

export default function LeadForm({ salesUsers = [], showAssignee, initialValues, onSubmit, onCancel }) {
  const initial = useMemo(() => ({ ...EMPTY, ...initialValues, assignedToId: initialValues?.assignedTo?.id ?? initialValues?.assignedToId ?? "" }), [initialValues]);
