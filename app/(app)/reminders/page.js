...
function CalendarBadge({ item }) {
  if (item.calendarEventId) return <span title="Synced to Outlook calendar" className="rounded bg-blue-100 px-1.5 py-0.5 text-[10px] font-semibold text-blue-700">📅 Synced</span>;
  if (item.calendarSyncError) return <span title={item.calendarSyncError} className="rounded bg-red-100 px-1.5 py-0.5 text-[10px] font-semibold text-red-700">📅 Sync failed</span>;
  return null;
}
