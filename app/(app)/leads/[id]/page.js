...
export default function LeadDetailPage({ params }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.65fr_0.9fr]"><div className="space-y-6">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-[var(--shadow-soft)] md:p-6"><h2 className="text-sm font-semibold text-slate-900">Emails</h2>{emailLogs.length === 0 ? <p className="mt-4 text-sm text-slate-400">No emails sent yet.</p> : <ul className="mt-4 space-y-3">{emailLogs.map((e) => <li key={e.id} className={`rounded-lg border p-4 text-sm ${e.direction === "INBOUND" ? "border-emerald-200 bg-emerald-50/50" : "border-slate-200 bg-slate-50"}`}>{e.direction === "INBOUND" && <span className="mb-1 inline-block rounded bg-emerald-100 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-emerald-700">Client reply</span>}<b className="block">{e.subject}</b><p className="mt-1 text-slate-500">{e.direction === "INBOUND" ? "from" : "to"} {e.toEmail}</p></li>)}</ul>}<div className="mt-5"><EmailForm defaultToEmail={lead.email} onSubmit={async (payload) => { await sendLeadEmail({ ...payload, leadId: id }); await load(); }}/></div></section>
